...
data class MDMEntity(
    @PrimaryKey var deviceName: String = Const.MDM.SETTING.DEFAULT_DEVICE_NAME,
    ...
    var rotation: Int = Const.MDM.SETTING.DEFAULT_ROTATION,
    ...
    var autoStartCameraActivity: Boolean = Const.MDM.SETTING.DEFAULT_AUTO_START_CAMERA_ACTIVITY,
...
class MDMRepo(private val context: Context, private val db: AppDataBase, private val mdmDao: MDMDao): BaseLocalRepo<MDMEntity>(){
    ...
    init {
        ...
        mdmService.connect(context, object : MDMService.ResultHandler {
            ...
            override fun onMDMConnected() {
                Log.d("MDM Repo onMDMConnected")
//                     MDMHandlers(context).init()
                syncMDMInfo()
            }

            override fun onMDMDisconnected() {
                Log.d("MDM Repo onMDMDisconnected")
            }
        })
    }
...
//            withTransaction {
//
//                val query = MDMService.getInstance().queryConfig(BuildConfig.MDM_API_KEY)
//                val mdmDeviceId = query.getString("DEVICE_ID")?:""
//                val deviceName = getDeviceName(mdmDeviceId)
...
//
//                baseMDMEntity.use_ageGender_NpuModel = (ageGenderModelType == "new")
//
//
//
//
//
//
//                val configJson = MDMService.Preferences.get("config_json", "")
//                val mdmEntity = MDMConverters.jsonToMdmEntity(configJson)
//
//                Log.d("syncMDMInfo $query")
//
//            }
    ...
    fun getDeviceName(mdmDeviceId: String): String {
        ...
        val baseName = (globalName?.takeIf { it.isNotBlank() } ?: Build.MODEL ?: Const.MDM.SETTING.DEFAULT_DEVICE_NAME).trim()
